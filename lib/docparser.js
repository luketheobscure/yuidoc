...
YUI.add('docparser', function (Y) {
    ...
    var Lang = Y.Lang,
         ...
         */
        TAGLIST = [
            "action",       // An action in the Ember action hash
        ...
        **/
        ...
        extract: function (filemap, dirmap) {
            ...
            Y.each(filemap, function (code, filename) {
                ...
                for (i = 0; i < len; i++) {
                    line = lines[i];
                    if (REGEX_START_COMMENT[syntaxtype].test(line) || REGEX_SINGLE_LINE_COMMENT[syntaxtype].test(line)) {
                        commentlines = [];
						...
						if(REGEX_START_COMMENT[syntaxtype].test(line)){
	                        ...
	                        if(line.match(REGEX_SINGLE_LINE_COMMENT[syntaxtype]).length > 1){
		                        commentlines.push('')
	                        	commentlines.push(line.match(REGEX_SINGLE_LINE_COMMENT[syntaxtype])[2]);
	                        }
                        }
                        ...
                        thisComment = this.handlecomment(comment, filename, linenum)
                        
                        if (REGEX_END_COMMENT[syntaxtype].test(line) || REGEX_SINGLE_LINE_COMMENT[syntaxtype].test(line)) {
	                        for (j = 0, thisLen = thisComment.length; j < thisLen; j++) {
		                        tag = thisComment[j]['tag']
		                        if(PRIMARY_TAGS.indexOf(tag) != -1){
			                        if (thisComment[j]['value'].match(REGEX_NAME_TYPE)){
                                        if (tag == "method") {
                                            thisComment.push(
                                                {tag: 'returns', value: thisComment[j]['value'].match(REGEX_NAME_TYPE)[1]}
                                            )
                                        } else {
                                            thisComment.push({tag: 'type', value: thisComment[j]['value'].match(REGEX_NAME_TYPE)[1]})
                                        }
			                        	if(thisComment[j]['value'].match(REGEX_NAME_TYPE)[1]){
				                            thisComment.push({tag: 'description', value: thisComment[j]['value'].match(REGEX_NAME_TYPE)[2]})
			                            }
                                        thisComment[j]['value'] = lines[i+1].match(REGEX_NAME)[0].trim()
			                        }
		                        }
		                    }
                        ...
                        commentmap[filename].push(thisComment);
                    }
                }
            }, this);
